...
export function assessLocalQuality(
  ...
  if (reasoning >= 65)
    strengths.push('Есть слова и конструкции, связывающие аргументы и выводы.');
  if (specificity >= 65)
    strengths.push('Есть числа, перечисления или другие конкретные детали.');
  if (calibration >= 65)
    strengths.push('В тексте встречаются оговорки и обозначения ограничений.');
  if (evidence < 55)
    limitations.push(
  ...
  return {
    ...
    summary:
      score >= 70
        ? 'В тексте много признаков источников, пояснений и конкретных деталей. Это не проверка достоверности.'
        : score >= 50
          ? 'В тексте есть отдельные признаки пояснений или источников. Их корректность не проверена.'
          : 'Локальный анализ нашёл мало явных признаков пояснений и источников. Это не означает, что текст неверен.',
