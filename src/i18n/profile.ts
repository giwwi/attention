...
import { OPTIONAL_AI_ROWS } from './optional-ai';
...
Готовим запрос для {provider}…|Preparing the prompt for {provider}…|Anfrage für {provider} wird vorbereitet…|Preparando el mensaje para {provider}…|Préparation de la demande pour {provider}…|Preparazione della richiesta per {provider}…|正在准备 {provider} 提示词…|جارٍ إعداد الطلب لـ {provider}…|{provider} के लिए प्रॉम्प्ट तैयार हो रहा है…
...
Запрос скопирован, {provider} открыт.|Prompt copied; {provider} is open.|Anfrage kopiert; {provider} ist geöffnet.|Mensaje copiado; {provider} abierto.|Demande copiée ; {provider} est ouvert.|Richiesta copiata; {provider} aperto.|提示词已复制，{provider} 已打开。|نُسخ الطلب وفُتح {provider}.|प्रॉम्प्ट कॉपी हो गया; {provider} खुला है।
Запрос уже скопирован. Откройте {provider} кнопкой ниже, вставьте его в поле сообщения и отправьте.|Prompt copied. Open {provider} below, paste the prompt and send it.|Anfrage kopiert. {provider} unten öffnen, einfügen und senden.|Mensaje copiado. Abre {provider} abajo, pégalo y envíalo.|Demande copiée. Ouvrez {provider} ci-dessous, collez et envoyez.|Richiesta copiata. Apri {provider}, incolla e invia.|提示词已复制。请打开下方 {provider}，粘贴并发送。|نُسخ الطلب. افتح {provider} أدناه والصقه وأرسله.|प्रॉम्प्ट कॉपी हो गया। नीचे {provider} खोलकर पेस्ट करें और भेजें।
...
export function profileText(
  ...
  const providerLabel = source.match(
    /^Пусть (ChatGPT|Claude|Gemini|Copilot|Perplexity) представит меня$/u,
  );
  if (providerLabel)
    return profileText(
