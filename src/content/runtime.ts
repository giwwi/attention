import {
  ...
  HIGHLIGHT_SECTIONS_MESSAGE_TYPE,
  ...
  type HighlightSectionsMessage,
  type HighlightSectionsResponse,
  type PageCapture,
...
import { isCardOpenMessage } from '../shared/card-messages';
...
export function startContentRuntime(): () => void {
  ...
  let readingAttemptVersion = 0;
  let suppressedCandidateUrl: string | null = null;
  ...
  function setReadingCandidate(capture: PageCapture): void {
    if (suppressedCandidateUrl === canonicalUrl(capture.url)) return;
  ...
  function ensureCurrentReadingCandidate(): void {
    if (suppressedCandidateUrl === canonicalUrl(window.location.href)) return;
  ...
  async function maybeStartReadingCandidate(): Promise<void> {
    ...
    if (
      ...
      (!progressedInsideArticle &&
        window.scrollY < candidate.initialScrollY + 64)
    ) {
      return;
    }
    autoStartPending = true;
    const version = readingAttemptVersion;
    try {
      const response: unknown = await chrome.runtime.sendMessage({
        type: ATTENTION_SESSION_AUTO_START_TYPE,
        capture: candidate.capture,
      });
      if (version === readingAttemptVersion && isAutoStartResponse(response))
        startTracker(response.session);
    } finally {
      if (version === readingAttemptVersion) autoStartPending = false;
    }
  }
  ...
  function synchronizePageState(): void {
    ...
    stopTracker(false);
    readingCandidate = null;
    readingAttemptVersion += 1;
    autoStartPending = false;
    suppressedCandidateUrl = null;
...

  {
    ...
    const messageListener: Parameters<
      typeof chrome.runtime.onMessage.addListener
    >[0] = (rawMessage: unknown, sender, sendResponse) => {
      if (isCardOpenMessage(rawMessage)) {
        // Only the extension's own popup can request a focused article card.
        // A page-generated DOM event is never a command to open or act on it.
        if (
          sender.id !== chrome.runtime.id ||
          sender.url?.split(/[?#]/u)[0] !==
        ...
        void hoverPreviewController
          .openCurrentArticle()
          .then(sendResponse)
          .catch(() => sendResponse({ ok: false, reason: 'unavailable' }));
        return true;
      }
      if (isAttentionInputsInvalidatedMessage(rawMessage)) {
        if (rawMessage.changedKeys.includes('attentionDataGeneration')) {
          readingAttemptVersion += 1;
          autoStartPending = false;
          readingCandidate = null;
          suppressedCandidateUrl = canonicalUrl(window.location.href);
          stopTracker(false);
          outcomePrompt.hide();
        }
        readingPageCache.invalidate();
      ...
      if (message?.type === HIGHLIGHT_SECTIONS_MESSAGE_TYPE) {
        const response: HighlightSectionsResponse = {
          ok: true,
          highlighted: highlightRecommendedSections(document, message.headings),
        };
        sendResponse(response);
        return;
      }
  ...
  async function decideOnCurrentArticle(
    capture: PageCapture,
    decision: MaterialDecision,
  ): Promise<boolean> {
    if (canonicalUrl(capture.url) !== canonicalUrl(window.location.href))
      return false;
    readingAttemptVersion += 1;
    const decisionVersion = readingAttemptVersion;
    autoStartPending = false;
    const response: AttentionMaterialDecideResponse =
      await chrome.runtime.sendMessage({
        type: ATTENTION_MATERIAL_DECIDE_TYPE,
        capture,
        decision,
      });
    if (
      !response?.ok ||
      decisionVersion !== readingAttemptVersion ||
      canonicalUrl(capture.url) !== canonicalUrl(window.location.href)
    )
      return false;
    if (decision === 'read' || decision === 'skim') {
      if (!isAutoStartResponse(response)) return false;
      suppressedCandidateUrl = null;
      startTracker(response.session);
    } else {
      suppressedCandidateUrl = canonicalUrl(capture.url);
      readingCandidate = null;
      stopTracker(false);
      outcomePrompt.hide();
    }
  ...
  hoverPreviewController = installHoverPreview({
    onDecision: decideOnCurrentArticle,
  ...
  return () => {
    ...
    contentScriptGlobal.__pageCaptureListenerInstalled = false;
    outcomePrompt.hide();
    document
      .querySelectorAll(
        '[data-attention-outcome-prompt], [data-attention-profile-handoff-notice]',
      )
